 ...
 */
import { PrismaClient, GlobalRole, GrantStatus, GrantVia, SystemStatus, UserStatus } from '@prisma/client';
...
function genApiKey(systemTag: string): { raw: string; hash: string; prefix: string } {
...
async function main() {
  ...
  for (const u of TEST_ACCOUNTS) {
    ...
    const created = await prisma.user.upsert({
      ...
      update: {
        ...
        global_role: u.global_role as GlobalRole,
  ...
  const todos = [
    { owner_email: 'jerry.sun@mrdi.org.hk', source: 'mdm-api',     title: 'Review Sprint 2 MDM API 验收清单',   label: 'red'   as const, related_id: 'sprint-2' },
    { owner_email: 'editor@mrdi.org.hk',    source: 'cimrms-api',   title: '跟进 cimrms-api RBAC 进度',          label: 'green' as const, related_id: 'sprint-2' },
    { owner_email: 'jerry.sun@mrdi.org.hk', source: 'mrdi-portal', title: '本周 portal 登录 + CORS 验证',       label: 'blue'  as const, related_id: 'sprint-2' },
  ];
