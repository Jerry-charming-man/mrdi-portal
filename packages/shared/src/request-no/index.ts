...
export function formatRequestNo(prefix: RequestNoPrefix, year: number, seq: number): string {
  return `${prefix}-${year}-${seq.toString().padStart(4, '0')}`;
}

export function parseRequestNo(no: string): { prefix: RequestNoPrefix; year: number; seq: number } | null {
  const match = /^(NC|INC|PRM)-(\d{4})-(\d{4,})$/.exec(no);
  if (!match) return null;
  return {
    prefix: match[1] as RequestNoPrefix,
