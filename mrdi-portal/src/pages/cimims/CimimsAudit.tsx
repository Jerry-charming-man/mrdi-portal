...
import { MOCK_AUDIT_LOGS } from '../../types/cimims'

const typeColors: Record<string, string> = {
  status_change: 'bg-warn-soft text-warn',
  takeover: 'bg-ignite-soft text-ignite',
  transfer: 'bg-pink/10 text-pink',
  close: 'bg-success-soft text-success',
  escalation: 'bg-danger-soft text-danger',
  comment: 'bg-module text-ink-3',
}
...
export default function CimimsAudit() {
    ...
    const t = setInterval(() => {
      setLogs(prev => {
        const newLog = {
          ...
          incidentId: 'i1',
          incidentNo: 'INC-2026-0287',
          event: '状态变更',
          type: 'status_change' as const,
          actor: 'system',
          actorName: '系统',
          detail: 'pending_takeover → processing',
        }
        return [newLog, ...prev]
      })
    }, 8000)
  ...
  if (search) {
    ...
    filtered = filtered.filter(l => l.incidentNo.toLowerCase().includes(q) || l.actorName.toLowerCase().includes(q) || l.event.toLowerCase().includes(q))
  }
    ...
    <div className="px-8 py-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[13px] text-ink-3 mt-1">事件流转全记录 · 共 {filtered.length} 条</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => toast('CSV 导出在 Sprint 2 上线')}
          ...
          <button onClick={() => setLogs(prev => [...prev])}
            className="px-4 py-2 rounded-lg border border-progress text-[13px] hover:bg-module flex items-center gap-1.5">
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="relative flex-1 max-w-sm">
          <svg className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-ink-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <input type="text" placeholder="搜事件编号 / 操作人 / 事件…"
      ...
      <div className="bg-pure rounded-2xl shadow-card border border-progress/60 overflow-hidden">
        <table className="w-full text-[13px]">
          ...
          <tbody className="divide-y divide-progress">
            {filtered.map((log) => {
              const typeCfg = typeColors[log.type] || typeColors.comment
              const typeLabel = typeLabels[log.type] || log.type
              return (
                <>
                  <tr key={log.id} className="row-hover cursor-pointer" onClick={() => setExpanded(expanded === log.id ? null : log.id)}>
                    <td className="px-5 py-3 font-mono text-[12px] text-ink-3">{formatTime(log.timestamp)}</td>
                    <td className="px-3 py-3 font-mono font-medium text-ignite">{log.incidentNo}</td>
                    ...
                    <tr key={`${log.id}-detail`} className="bg-module">
                      <td colSpan={5} className="px-5 py-3">
                        <code className="bg-pure px-2 py-1 rounded text-[11px] font-mono text-ink-2">
                          {JSON.stringify({ incidentNo: log.incidentNo, event: log.event, actor: log.actorName, detail: log.detail, timestamp: log.timestamp }, null, 2)}
                        </code>
                      </td>
                    </tr>
                  )}
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-12 text-center text-ink-3">没有找到匹配的日志</td></tr>
            )}
          </tbody>
        </table>
      </div>
