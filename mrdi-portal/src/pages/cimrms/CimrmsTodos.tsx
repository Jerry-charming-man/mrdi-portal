...
import { MOCK_TODOS, URGENCY } from '../../types/cimrms'
import type { CimrmsTodo } from '../../types/cimrms'
...
export default function CimrmsTodos() {
  const [todos, setTodos] = useState<CimrmsTodo[]>(MOCK_TODOS)
  const [showAdd, setShowAdd] = useState(false)
  const [newTitle, setNewTitle] = useState('')
  const [newLabel, setNewLabel] = useState<CimrmsTodo['label']>('green')
  ...
  let filtered = openTodos
  if (filterLabel !== 'all') filtered = filtered.filter(t => t.label === filterLabel)
  ...
  return (
    <div className="px-8 py-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Red', count: redCount, color: 'text-danger', dot: 'bg-danger', desc: '紧急处理' },
      ...
      <div className="bg-pure rounded-2xl shadow-card border border-progress/60 overflow-hidden">
        <div className="divide-y divide-progress">
          {filtered.map((todo) => {
            ...
            const urgencyCfg = URGENCY[todo.priority as keyof typeof URGENCY] || URGENCY.P3
            return (
              <div key={todo.id} className="px-5 py-3.5 flex items-center gap-3 hover:bg-module transition">
                ...
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-pure font-mono ${urgencyCfg.bg}`}>
                  {urgencyCfg.code}
                </span>
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg border border-progress text-[12px] text-ink-3 hover:bg-module"
                  >
                    Dismiss
                  </button>
                  <button
                    onClick={() => handleDone(todo.id)}
                    className="px-2.5 py-1.5 rounded-lg bg-ignite text-pure text-[12px] font-medium hover:bg-ignite-2"
                  >
                    Done
                  </button>
                </div>
