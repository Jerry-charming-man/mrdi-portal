...
import { MOCK_EXCEPTIONS } from '../../types/cimrms'

export default function CimrmsExceptions() {
  ...
  const slaBreach = exceptions.filter(e => e.type === 'sla_breach').length
  const sla50 = exceptions.filter(e => e.type === 'sla_50').length
  const rejectedLoop = exceptions.filter(e => e.type === 'rejected_loop').length
  ...
  function getSlaPercent(ex: typeof MOCK_EXCEPTIONS[0]) {
    return Math.min(100, Math.round((ex.hoursElapsed / ex.slaHours) * 100))
  }

  function getSlaColor(ex: typeof MOCK_EXCEPTIONS[0]) {
    const pct = getSlaPercent(ex)
  ...
  return (
    <div className="px-8 py-6">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-[24px] font-semibold tracking-tight">异常处理</h1>
        <p className="text-[13px] text-ink-3 mt-1">
          {slaBreach} 项 SLA 突破 · {sla50} 项 SLA 50% 警告 · {rejectedLoop} 项驳回循环 · 需立即处理
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <SeverityCard
          label="SLA 突破"
          count={slaBreach}
          color="bg-danger-soft border-danger"
          desc="超过 SLA 时限"
        />
        <SeverityCard
          label="SLA 50% 警告"
          count={sla50}
          color="bg-warn-soft border-warn"
          desc="已消耗 50% SLA"
        />
        <SeverityCard
          label="驳回循环 ≥2"
          count={rejectedLoop}
          color="bg-ink-3/10 border-ink-3"
          desc="被驳回 2 次以上"
        />
      </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`w-4 h-4 rounded flex items-center justify-center ${isCritical ? 'text-danger' : 'text-warn'}`}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                        </svg>
                      </span>
                      <span className="font-mono text-[12px] text-ink font-medium">{ex.requestNo}</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold text-pure ${
                        ex.type === 'sla_breach' ? 'bg-danger' : ex.type === 'sla_50' ? 'bg-warn' : 'bg-ink-3'
                      }`}>
                        {ex.type === 'sla_breach' ? 'SLA 突破' : ex.type === 'sla_50' ? 'SLA 50%' : '驳回循环'}
                      </span>
                    ...
                    <div className="mt-2">
                      <div className="flex items-center gap-2 text-[11px] text-ink-3 mb-1">
                        <span>SLA 进度</span>
                        <span className="font-mono">{ex.hoursElapsed}h / {ex.slaHours}h</span>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-[12px] text-ink-3">
                      ...
                      <span>已耗时: <span className={`font-mono ${ex.hoursElapsed > ex.slaHours ? 'text-danger' : 'text-warn'}`}>{ex.hoursElapsed}h</span></span>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      onClick={() => { const name = ex.assignee || '处理人'; toast(`提醒已发送给 ${name}`) }}
                    ...
                    <button
                      onClick={() => setShowEscalateModal(ex.id)}
                    ...
                    <Link
                      to={`/cimrms/requests/${ex.requestId}`}
                      className="px-3 py-1.5 rounded-lg border border-progress text-[12px] text-ignite hover:bg-module"
                    >
                      查看详情 →
                    </Link>
                  </div>
      ...
      {/* Escalate Modal */}
      {showEscalateModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowEscalateModal(null)}>
          <div className="bg-pure rounded-2xl shadow-card-hover p-6 w-[400px]" onClick={e => e.stopPropagation()}>
            <h3 className="text-[16px] font-semibold text-ink mb-1">升级处理</h3>
            <p className="text-[13px] text-ink-3 mb-4">将此需求升级给更高级别处理</p>
            <div className="space-y-3 mb-5">
              <div>
                ...
                <select className="w-full px-3 py-2.5 bg-module border border-progress rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-ignite/30">
                  <option>IT-CIM Lead（默认）</option>
                  <option>IT-CIM Manager</option>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => { setShowEscalateModal(null); toast('已升级给 IT-CIM Lead') }}
