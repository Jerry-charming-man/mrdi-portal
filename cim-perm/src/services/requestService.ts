...
// Uses SELECT FOR UPDATE on a dedicated sequence table for true atomicity
// under high concurrency (30+ concurrent inserts). Retry on unique key violation.
// Migration (run once):
...
async function nextRequestNo(): Promise<string> {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      const row = await queryOne<{ last_seq: number }>(
        `INSERT INTO perm_request_seq (year, last_seq)
           VALUES ($1, 1)
           ON CONFLICT (year) DO UPDATE SET last_seq = perm_request_seq.last_seq + 1
           RETURNING last_seq`,
        [year]
      )
      return prefix + (row!.last_seq).toString().padStart(4, '0')
    } catch (err: unknown) {
      const e = err as { code?: string; message?: string }
      // 23505 = unique constraint violation on permission_request.request_no
      // This means two transactions got the same seq → retry
  ...
  throw new Error(`Failed to generate unique request_no after ${MAX_RETRIES} attempts`)
}
...
export async function createRequest(
  ...
  // Retry on unique constraint violation (request_no duplicate under high concurrency).
  // Each retry: new uuid + new seq → fresh attempt. Very rare with uuid v4 + seq retry.
  const MAX_RETRIES = 5
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    const id         = uuidv4()
    ...
    try {
      ...
      const e = err as { code?: string; message?: string }
      if (e.code === '23505' || (e.message && e.message.includes('unique constraint'))) {
        continue // retry with fresh uuid + seq
      }
      throw err
    }
  }
  throw new Error('Failed to create permission request after max retries')
}
...
export async function itReview(
  ...
  // Use isRejected flag to avoid PostgreSQL parameter $1 type conflict
  // (reusing $1 in both SET and CASE WHEN causes type inference failure)
  ...
  const row = await queryOne<PermRequestRow>(
    `UPDATE permission_request SET
     ...
     WHERE id = $6
     RETURNING *`,
    [newStatus, user.email, user.name, isRejected ? (body.comment ?? null) : null, isRejected ? new Date() : null, id]
  )
